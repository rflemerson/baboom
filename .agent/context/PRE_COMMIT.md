...
-   **Config**: `apps/api/pyproject.toml` (`[tool.ruff]`).
-   **Target**: Python 3.14.
-   **Rules**:
...
-   **Config**: `apps/api/pyproject.toml` (`[tool.djlint]`).
-   **Target**: `.html` files.
-   **Profile**: `django`.
...
2.  **Fix Issues**:
    ...
    -   **MyPy**: Add type hints.
    -   **Deptry**: Remove unused packages from `apps/api/pyproject.toml`.
