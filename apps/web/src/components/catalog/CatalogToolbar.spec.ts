...
describe('CatalogToolbar', () => {
  it('emits search, sort, per-page, drawer, clear, and direction events', async () => {
    ...
    await buttons.find((button) => button.text() === 'Advanced filters')?.trigger('click')
    await buttons.find((button) => button.text() === 'Ascending')?.trigger('click')
    await buttons.find((button) => button.text() === 'Grid')?.trigger('click')
    await buttons.find((button) => button.text() === 'List')?.trigger('click')
    await buttons.find((button) => button.text() === 'Clear filters')?.trigger('click')
    ...
    expect(wrapper.emitted('update:viewMode')?.[1]).toEqual(['list'])
